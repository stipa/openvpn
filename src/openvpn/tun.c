...
service_register_ring_buffers(const struct tuntap *tt)
{
    ...
    {
        msg(M_WARN, "Register ring buffers failed using service: %s [status=0x%x]",
            strerror_win32(ack.error_number, &gc), ack.error_number);
        res = false;
    }
    else
...
open_tun(const char *dev, const char *dev_type, const char *dev_node, struct tuntap *tt)
{
    ...
    if (tt->wintun)
    {
        ...
        if (tt->options.msg_channel)
        {
            if (!service_register_ring_buffers(tt))
            {
                msg(M_FATAL, "ERROR:  Failed to register ring buffers via service: %lu", GetLastError());
            }
        }
        else
        {
