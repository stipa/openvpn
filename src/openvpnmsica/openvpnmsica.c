...
#define FILE_NEED_REBOOT        L".ovpn_dco_need_reboot"
...
cleanup_hRecord:
    ...
    /* save path to user temp dir to be used by deferred action */
    WCHAR tmpDir[MAX_PATH];
    GetTempPath(MAX_PATH, tmpDir);

    WCHAR str[MAX_PATH + 7];
    swprintf_s(str, _countof(str), TEXT("tmpdir=%") TEXT(PRIsLPTSTR), tmpDir);
...
static void
CreateRebootFile(_In_z_ LPCWSTR pathToTmp)
{
    TCHAR path[MAX_PATH];
    swprintf_s(path, _countof(path), L"%s%s", pathToTmp, FILE_NEED_REBOOT);
...
UINT __stdcall
CheckReboot(_In_ MSIHANDLE hInstall)
{
#ifdef _MSC_VER
#pragma comment(linker, DLLEXP_EXPORT)
#endif
    ...
    if (FindFirstFile(path, &data) != INVALID_HANDLE_VALUE)
    {
